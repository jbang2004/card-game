...
const bar=document.createElement('section');bar.id='vfxlab';bar.innerHTML=`<header><strong>EMBERFALL / R8 · SPECTRAL REFINEMENT</strong><span id="lab-mode">真实对局 · 独立练习</span><button class="close" id="lab-close">收起 ×</button></header><div class="skills"><select id="lab-role" aria-label="选择角色与剑技" style="flex:2;min-width:0">${catalog.map(id=>`<option value="${id}">${titleFor(id)}</option>`).join('')}</select><button id="lab-cast">实际出招</button><button data-fx="breath">原版喷火</button><button id="lab-audio">声音：关</button></div><div class="row"><button id="lab-play" title="重播上一次的特效，不重复结算伤害">▶</button><button id="lab-step" title="逐帧">▸│</button><input id="lab-time" type="range" value="0" min="0" max="1000" step="1" aria-label="特效时间轴"><span class="readout" id="lab-readout">0.00s</span><select id="lab-speed" aria-label="慢放速度"><option value="1" selected>1×</option><option value=".5">½×</option><option value=".25">¼×</option></select><button id="lab-cycle">连播</button></div><p class="note">R8：圣裁锋面 · 暗影切口 · 晶体霜刃。实际攻击只结算一次；回看同步卡牌姿态与特效。</p>`;
...
fixture('paladin');title.innerHTML='三式 · 形与力<small>R8 / HOLY · SHADOW · FROST</small>';ui();
})();
