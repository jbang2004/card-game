...
const bar=document.createElement('section');bar.id='vfxlab';bar.innerHTML=`<header><strong>EMBERFALL / R3 · FLOW & IMPACT</strong><span id="lab-mode">真实对局 · 独立练习</span><button class="close" id="lab-close">收起 ×</button></header><div class="skills">${Object.entries(names).map(([k,v])=>`<button data-fx="${k}">${v}</button>`).join('')}</div><div class="row"><button id="lab-play" title="重播上一次的特效，不重复结算伤害">▶</button><button id="lab-step" title="逐帧">▸│</button><input id="lab-time" type="range" value="0" min="0" max="1000" step="1" aria-label="特效时间轴"><span class="readout" id="lab-readout">0.00s</span><select id="lab-speed" aria-label="慢放速度"><option value="1" selected>1×</option><option value=".5">½×</option><option value=".25">¼×</option></select><button id="lab-cycle">连播</button></div><p class="note">R3：火舌独立翻卷，重剑回引后挥落。上排执行真实攻击；时间轴只回看特效，不重复扣血。</p>`;
...
fixture('breath');title.innerHTML='R3 · 焰流与重劈<small>ADVECTED FIRE / OVERHEAD CLEAVE</small>';ui();
})();
