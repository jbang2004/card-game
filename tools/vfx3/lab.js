...
const extraRoles=['archer','huntress','wolf','berserker','golem','fireball','frostbolt','nova','renew','shield','blessing','silence','lifedrain','wisdom','wolves','rally','absolution','muster','execute','polymorph','tracking','demise'];
const catalog=[...extraRoles,'paladin','assassin','frostking',...roles.filter(x=>!['paladin','assassin','frostking'].includes(x)),'dagger','sunblade','breath','lightning'];
const titleFor=id=>{if(id==='demise')return '死亡 · 余魂离散';const card=EmberData.byId[id],style=EmberFXProfiles.swordIdentities[id]||({dagger:'twins',sunblade:'daybreak',sabotage:'shatter'})[id];
 return style?`${card?.name||id} · ${EmberSwordArts.get(style).name}`:(EmberData.byId[id]?.name||names[id]||id);};
...
const bar=document.createElement('section');bar.id='vfxlab';bar.innerHTML=`<header><strong>EMBERFALL / R9 · BATTLE REMASTER</strong><span id="lab-mode">真实对局 · 独立练习</span><button class="close" id="lab-close">收起 ×</button></header><div class="skills"><select id="lab-role" aria-label="选择角色与剑技" style="flex:2;min-width:0">${catalog.map(id=>`<option value="${id}">${titleFor(id)}</option>`).join('')}</select><button id="lab-cast">实际出招</button><button data-fx="breath">原版喷火</button><button id="lab-audio">声音：关</button></div><div class="row"><button id="lab-play" title="重播上一次的特效，不重复结算伤害">▶</button><button id="lab-step" title="逐帧">▸│</button><input id="lab-time" type="range" value="0" min="0" max="1000" step="1" aria-label="特效时间轴"><span class="readout" id="lab-readout">0.00s</span><select id="lab-speed" aria-label="慢放速度"><option value="1" selected>1×</option><option value=".5">½×</option><option value=".25">¼×</option></select><button id="lab-cycle">连播</button></div><p class="note">R9：弓矢 · 裂爪 · 重击 · 元素法术 · 生命仪式。实际攻击只结算一次；回看同步卡牌姿态与特效。</p>`;
...
function fixture(kind){
 ...
 const cid=id==='breath'?'dragon':(EmberData.byId[id]?.type==='minion'?id:'squire');
 for(const card of [cid,'archer','guard'])g.summon('p',card,{sick:false});
 for(const u of [...g.s.e.board,...g.s.p.board])u.hp=u.maxHp=60;
 if(EmberData.byId[id]?.type==='spell')g.s.p.hand=[g.card(id)];
 if(id==='lightning')g.s.p.hand=[g.card('bolt')];
 if(['renew','blessing'].includes(id)){g.s.p.board[0].hp=20;g.s.p.hp=20;}
 if(id==='demise'){g.s.p.hand=[g.card('bolt')];g.s.e.board[2].hp=1;}
 g.s.p.heroId=g.s.heroId||'mage';
...
function actionFor(kind){const g=EmberDebug.game;
 if(['sabotage','lightning','demise'].includes(kind)||EmberData.byId[kind]?.type==='spell')return {type:'play',side:'p',uid:g.s.p.hand[0].uid,target:['renew','blessing','shield','battlecry','dawnvow','soultether'].includes(kind)?{side:'p',uid:g.s.p.board[kind==='shield'?1:0].uid}:{side:'e',uid:g.s.e.board[2].uid}};
...
if(Number($('lab-speed').value)===1&&selected!=='demise'&&EmberFx2.mesh3d.last){
 ...
 EmberFx2.remasterFeedback?.play({...d,start:0,impact:d.impact-d.start},prev,t);
}
if(t>=duration)running=false;ui();}lastFrame=now;requestAnimationFrame(tick);}requestAnimationFrame(tick);
bar.querySelectorAll('[data-fx]').forEach(b=>b.onclick=()=>cast(b.dataset.fx));$('lab-play').onclick=play;$('lab-step').onclick=()=>seek(t+1000/60);$('lab-time').oninput=e=>seek(e.target.value);$('lab-cycle').onclick=()=>cast('archer',true);$('lab-cast').onclick=()=>cast($('lab-role').value);$('lab-role').value='archer';$('lab-role').onchange=()=>cast($('lab-role').value);
$('lab-speed').onchange=()=>{EmberAudio.stop();EmberFx2.benchmarkFeedback?.resetAudio();EmberFx2.remasterFeedback?.resetAudio();};
