...
const names={breath:'焰龙 · 灼息',lightning:'星火 · 闪电',slash:'天剑 · 贯星'};
...
const bar=document.createElement('section');bar.id='vfxlab';bar.innerHTML=`<header><strong>EMBERFALL / R5 · VERTICAL SKYFALL</strong><span id="lab-mode">真实对局 · 独立练习</span><button class="close" id="lab-close">收起 ×</button></header><div class="skills">${Object.entries(names).map(([k,v])=>`<button data-fx="${k}">${v}</button>`).join('')}</div><div class="row"><button id="lab-play" title="重播上一次的特效，不重复结算伤害">▶</button><button id="lab-step" title="逐帧">▸│</button><input id="lab-time" type="range" value="0" min="0" max="1000" step="1" aria-label="特效时间轴"><span class="readout" id="lab-readout">0.00s</span><select id="lab-speed" aria-label="慢放速度"><option value="1" selected>1×</option><option value=".5">½×</option><option value=".25">¼×</option></select><button id="lab-cycle">连播</button></div><p class="note">R5：屏幕外垂直贯入，卡面分叉裂纹；原版火焰不变。上排执行真实攻击；时间轴只回看特效，不重复扣血。</p>`;
