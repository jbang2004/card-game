...
ROOT=Path(__file__).resolve().parents[2];REC=ROOT/"output/benchmark-r7/recording"
OUT=ROOT/"output/benchmark-r7";VIDEO=OUT/"video";VIDEO.mkdir(exist_ok=True)
def find_font():
...
def main():
 ...
 for speed in [1,.5]:
  for c in m["clips"]:
   ...
   for idx in indices:
    ...
    mode="1× 原速 · 同步专属音效" if speed==1 else "½× 局部慢放 · 静音"
 ...
 cmd=["ffmpeg","-hide_banner","-loglevel","warning","-y","-framerate","30","-i",str(VIDEO/"%05d.jpg"),
      ...
      "-c:a","aac","-b:a","160k","-movflags","+faststart","-shortest",str(OUT/"Benchmark_Sword_Arts_R7.mp4")]
