...
const AtelierArt = (() => {
  ...
  const exact = Object.freeze(
    Object.fromEntries(EmberData.cards.map((c) => [c.id, c.id])),
  );
  const aliases = Object.freeze({
    ...
    queen: "treant",
    frost: "frostking",
    worldender: "ashdragon",
  });
  const heroPanels = Object.freeze({
    mage: "oracle",
    paladin: "paladin",
    ranger: "archer",
  });
  const bossPanels = Object.freeze({
    warden: "berserker",
    queen: "treant",
    oracle: "necromancer",
    frost: "frostking",
  ...
  function card(c) {
    if (!c?.id || !AnimeAssets[c.id])
      throw new Error(
        "Missing anime artwork for card: " + (c?.id || "<unknown>"),
      );
  ...
  function frameHero(h, context = "hero") {
    return framing(mapFor(h.art, h.palette, h.id), context);
  }
  const missing = EmberData.cards.filter((c) => !AnimeAssets[c.id]);
  if (missing.length)
    throw new Error(
      "Incomplete artwork release: " + missing.map((c) => c.id).join(", "),
    );
  EmberArt.url = url;
  EmberArt.card = card;
  EmberArt.portrait = url;
  ...
  return Object.freeze({
    ...
    focuses,
    get paintedCards() {
      return EmberData.cards.filter((c) => !!AnimeAssets[c.id]);
    },
    get cardFallbackCount() {
      return 0;
    },
  });
})();
