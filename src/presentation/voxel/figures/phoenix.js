...
EmberVoxelKit.define("phoenix", (() => {
  ...
  function build() {
    ...
    const PX = K.pixel;                                              // pixel-sprite variant: wings as a few big feather blocks
    ...
    for (const s of [1, -1]) sc.add(PX ? S.ell(0.009, 0.013, 0.014) : S.ell(0.006, 0.009, 0.012), { mat: "eye", p: [s * 0.043 * Hs, HC[1] + 0.006, HC[2] + 0.02 * Hs], r: [0, s * 0.5, 0], bone: "head", k: 0.002, vdil: 0.6 });
    ...
    if (!PX) sc.paint(S.custom((x, y, z) => (Math.floor((y + 0.9 * Math.abs(x)) / 0.0125) % 3 === 0 && z > 0.03 && y < 0.6 ? -1 : 1), [-1, -1, -1, 1, 1, 1]), { mat: "seam", soft: 0.001, only: ["plume"] });
    ...
    sc.add(S.torus(0.042 * f, PX ? 0.013 : 0.009), { mat: "gold", p: HP, r: [Math.PI / 2 - 0.3, 0, 0], bone: "chest", k: 0.002, vdil: 0.55 });
    ...
    for (const s of [1, -1]) {
      ...
      for (const c of [-1, 0, 1]) sc.limb([s * 0.05, 0.232, 0.062], [s * 0.05 + c * 0.02, 0.21, 0.1], PX ? 0.01 : 0.007, PX ? 0.005 : 0.0025, { mat: "goldD", bone: "foot" + n, k: 0.003, vdil: 0.5 });
      sc.limb([s * 0.05, 0.232, 0.058], [s * 0.05, 0.215, 0.03], PX ? 0.009 : 0.006, PX ? 0.005 : 0.0025, { mat: "goldD", bone: "foot" + n, k: 0.003, vdil: 0.5 });
    }
    ...
    sc.inPart("cloth", () => {
      for (const s of [1, -1]) {
        ...
        // (pixel: every other primary, fatter — four big feather blocks — and three broad secondaries)
        PRIM.forEach((tp, i) => {
          if (PX && i % 2) return;
          const t = w.x(tp), root = lerp(w.W, w.H, clamp(1 - i / 5, 0, 1)), mid = add(lerp(root, t, 0.5), [s * 0.018 * (i < 2 ? -1 : 1), 0.012, -0.008]);
          feather(sc, [root, mid, t], PX ? 0.05 : 0.03, PX ? 0.024 : 0.011, { bone: b3 });
        });
        // secondaries off the forearm, hanging down and out into the lower lobe of the wing
        const nSec = PX ? 3 : 6;
        for (let i = 0; i < nSec; i++) {
          const u = i / (nSec - 1), root = lerp(w.W, w.E, u), t = secTip(w, s, u);
          feather(sc, [root, add(lerp(root, t, 0.5), [s * 0.02, 0, 0]), t], PX ? 0.056 : 0.034, PX ? 0.026 : 0.012, { bone: b2 });
        }
        // tertials off the upper arm, shorter, tucking in toward the body
        for (let i = 0; i < (PX ? 2 : 3); i++) {
          const u = 0.25 + 0.35 * i, root = lerp(w.E, w.S, u), t = add(root, [s * (0.035 - 0.015 * i), -0.28 + 0.03 * i, -0.03]);
          feather(sc, [root, lerp(root, t, 0.5), t], PX ? 0.05 : 0.034, PX ? 0.024 : 0.013, { bone: b1 });
        }
        ...
        const tips = PX ? [...PRIM.filter((_, i) => !(i % 2)).map(w.x), ...[0, 1, 2].map((i) => secTip(w, s, i / 2))] : [...PRIM.map(w.x), ...[0, 1, 2, 3, 4, 5].map((i) => secTip(w, s, i / 5))];
        sc.paint(S.custom((x, y, z) => Math.max(0.3 - armD(x, y, z), side(x)), [-1, -1, -1, 1, 1, 1]), { mat: "tip", soft: 0.002, only: ["feather"] });
        for (const t of tips) sc.paint(S.sphere(PX ? 0.06 : 0.05), { mat: "tip", p: t, soft: 0.002, only: ["feather", "light", "blue", "deep"] });
      }
      ...
      (PX ? [[-0.12, 0.04, -0.21], [0, -0.005, -0.25], [0.12, 0.04, -0.21]] : [[-0.16, 0.06, -0.19], [-0.08, 0.02, -0.23], [0, -0.005, -0.25], [0.08, 0.02, -0.23], [0.16, 0.06, -0.19]]).forEach(([x, y, z], i) => {
        const root = [x * 0.12, 0.34, -0.06], c = Math.sign(x) || 1, tip = [x + c * 0.045, y + 0.03, z + 0.03];
        feather(sc, [root, [x * 0.45, 0.22, z * 0.7], [x * 0.9, y + 0.07, z], tip], PX ? 0.036 : 0.024, PX ? 0.02 : 0.011, { mat: "feather", bone: "tail2" }).wfn = twf;
    ...
    sc.inPart("hair", () => {
      (PX ? [[0, 0.13, "gold"], [0.034, 0.1, "plumeB"], [-0.034, 0.1, "plumeB"]] : [[0, 0.12, "gold"], [0.028, 0.1, "plume"], [-0.028, 0.1, "plume"], [0.045, 0.075, "plumeB"], [-0.045, 0.075, "plumeB"]]).forEach(([x, L, mat]) => {
        const r = [x, HC[1] + 0.04 * Hs, HC[2] - 0.01];
        const pts = spline([r, [x * 1.4, r[1] + L * 0.6, r[2] - L * 0.4], [x * 1.8, r[1] + L * 0.9, r[2] - L * 1.0], [x * 2.1, r[1] + L * 0.95, r[2] - L * 1.4]], 10).map((p, i) => [...p, PX ? mix(0.022, 0.009, i / 10) : mix(0.016, 0.004, i / 10)]);
        sc.add(S.chain(pts, 0.6), { mat, p: [0, 0, 0], bone: "head", k: 0.004, vdil: 0.55 });
      });
      // cheek feathers sweeping back from behind the eyes
      if (!PX) for (const s of [1, -1]) sc.add(S.chain(spline([[s * 0.045 * Hs, HC[1] - 0.01, HC[2] - 0.01], [s * 0.056 * Hs, HC[1] - 0.004, HC[2] - 0.05], [s * 0.052 * Hs, HC[1] + 0.01, HC[2] - 0.09]], 6).map((p, i) => [...p, mix(0.012, 0.003, i / 6)])), { mat: "plumeB", p: [0, 0, 0], bone: "head", k: 0.004, vdil: 0.55 });
    });
