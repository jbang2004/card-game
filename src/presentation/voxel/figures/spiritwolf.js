...
EmberVoxelKit.define("spiritwolf", (() => {
  ...
  function spiritwolf() {
    ...
    const PX = K.pixel;                                              // pixel-sprite variant: clean big shapes, few fat flames
    ...
    mats(sc, {
      ...
      furFace: { c: PX ? 0x2a7a5a : 0x52ad7e, rough: 0.8, emit: 0.18, cls: CLS.fur, vary: 0.06, fur: 0.25 }, muzzle: { c: 0xb8f0cc, rough: 0.8, emit: 0.3, cls: CLS.fur, vary: 0.04, fur: 0.2 },
    ...
    for (const s of [1, -1]) sc.limb(add(hc, [s * 0.012 * Hs, -0.024 * Hs, (0.03 + mzL * 0.78) * Hs]), add(hc, [s * 0.012 * Hs, -0.04 * Hs, (0.03 + mzL * 0.8) * Hs]), PX ? 0.008 : 0.004, PX ? 0.004 : 0.0015, { mat: "fang", bone: "head", k: 0.002, vdil: 0.5 });
    // glowing slanted eyes under a heavy brow (the pixel sprite paints its eyes on the face decal instead)
    if (PX) sc.face({ bone: "head", cls: [5], c: add(hc, [0, 0.004 * Hs, 0.047 * Hs]), size: [0.05 * Hs, 0.04 * Hs], depth: 0.03, nz: 0.2 });
    for (const s of [1, -1]) {
      const eK = PX ? 1.9 : 1;
      if (!PX) sc.add(S.ell(0.011 * Hs * eK, 0.0055 * Hs * eK, 0.007 * Hs * (PX ? 1.4 : 1)), { mat: "eye", p: add(hc, [s * (PX ? 0.023 : 0.021) * Hs, 0.004 * Hs, (PX ? 0.047 : 0.042) * Hs]), r: [0, s * 0.35, s * 0.35], bone: "head", k: 0.003, cs: 0.03 });
      sc.limb(add(hc, [s * 0.008 * Hs, (PX ? 0.017 : 0.014) * Hs, 0.046 * Hs]), add(hc, [s * 0.034 * Hs, (PX ? 0.023 : 0.02) * Hs, 0.036 * Hs]), (PX ? 0.009 : 0.006) * Hs, (PX ? 0.008 : 0.005) * Hs, { mat: "furDark", bone: "head", k: 0.004 });
    }
    // ears laid back a little
    for (const s of [1, -1]) {
      const eK = PX ? 1.2 : 1;
      const n = sideName(s), base = add(hc, [s * 0.028 * Hs, 0.024 * Hs, -0.01]), tip = add(base, [s * 0.018 * Hs * eK, 0.058 * Hs * eK, -0.03]);
      sc.limb(base, tip, 0.022 * Hs * eK, PX ? 0.004 : 0.0018, { mat: "furFace", bone: "ear" + n, k: 0.012, sz: 0.42 });
    ...
    for (const s of [1, -1]) {
      ...
      if (!PX) for (const [P4, bn] of [[F[3], "fpaw" + n], [B[3], "bpaw" + n]]) for (let c = -1; c <= 1; c++) sc.add(S.ell(0.003, 0.003, 0.005), { mat: "claw", p: add(P4, [c * 0.009 * Ps, -0.008, 0.034 * Ps]), bone: bn, k: 0.003, cs: 0.05 });
    }
    // flame wisps: tapered curls streaming up and back, pale at the tip
    const wisp = (base, ctrl, r, bone, bones) => {
      const pts = spline([base, ...ctrl], 12).map((p, j) => [p[0], p[1], p[2], mix(r, PX ? 0.006 : 0.0015, Math.pow(j / 12, 1.1))]);
    ...
    if (PX) {
      // pixel: three big flame locks along the neck and back, and a flame at each shoulder
      [[G.chest, 0.066, 0.07, "chest", -1], [G.spine, 0.055, 0.07, "spine", 1], [lerp(G.spine, G.root, 0.6), 0.05, 0.06, "root", -1]].forEach(([p, h, len, bone, sd]) => {
        const b = add(p, [0, h - 0.014, 0]);
        wisp(b, [add(b, [sd * 0.012, len * 0.7, -len * 0.35]), add(b, [-sd * 0.006, len * 1.05, -len * 1.2])], 0.032, bone);
      });
    } else ridge.forEach(([p, h, len, bone], i) => {
    ...
    for (let i = 0; i <= 14; i++) { const u = i / 14; tp.push([PX ? 0 : 0.015 * Math.sin(u * 5), T0[1] + (PX ? -0.01 : 0.13) * u - (PX ? 0.06 : 0.02) * u * u, T0[2] - (PX ? 0.24 : 0.2) * u + 0.05 * u * u, ((0.022 + 0.02 * Math.sin(u * 2.8)) * (1 - 0.8 * u * u) + 0.003) * (PX ? 1.3 : 1)]); }
    ...
    tail.wfn = tw;
    for (let i = 0; i < (PX ? 2 : 4); i++) {
      if (PX) { const b = tp[7 + i * 3].slice(0, 3), sd = i % 2 ? 1 : -1; wisp(b, [add(b, [sd * 0.025, 0.04, -0.02]), add(b, [sd * 0.012, 0.08, -0.06])], 0.024, "tail2").wfn = tw; continue; }
    ...
    if (!PX) sc.paint(S.rcone(0.05 * Hs, 0.007 * Hs, 0.004 * Hs), { mat: "muzzle", p: add(hc, [0, -0.004 * Hs, 0.045 * Hs]), R: rotY2(0, 1, -0.55), soft: 0.005, fur: 0.25 });
