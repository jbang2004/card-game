...
EmberVoxelKit.define("jingchen", (() => {
  ...
  function hairOf(sc, P) {
    ...
    for (let i = 0; i < 4; i++) {
      const u = i / 3, s = i < 2 ? -1 : 1;
      const root = [0.01, C[1] + R[1] * 0.72, C[2] + R[2] * 0.58];
      const mid = [s * R[0] * (0.3 + 0.25 * u), ey + 0.058, P.faceZ + 0.004 * q];
      const tip = [s * R[0] * (0.7 + 0.25 * u), ey + 0.0 - 0.02 * u, P.faceZ - 0.018 * q];
      strand(sc, [root, mid, tip], 0.012 * q, 0.002 * q, { k: 0.007, taper: 1.2, grooves: 0.001 });
    }
    ...
    for (const s of [1, -1]) {
      ...
      const sp = strand(sc, [root, a, b, tip], 0.013 * q, 0.003 * q, { wave: 0.006, wd: [s, 0, 0.3], waves: 2, phase: s, k: 0.01, wg: 4, bone: "head" });
    ...
    const N = 8;
    for (let i = 0; i < N; i++) {
      const u = (i / (N - 1)) * 2 - 1, az = Math.PI + u * 1.55, el = 0.35 - 0.25 * Math.abs(u);
      ...
      const sp = strand(sc, [root, out, mid, tip], (0.02 - 0.004 * Math.abs(u)) * q, 0.004 * q, { wave: 0.007, wd: [Math.cos(az), 0, 0], waves: 2, phase: i * 1.7, k: 0.012, wg: 5, swell: 0.2, bone: "hairB1", grooves: 0.0012 });
  ...
  function build(fam) {
    ...
    for (const s of [1, -1]) {
      ...
      for (const [dx, dz, len] of [[0.02, 0.02, 0.07], [0.035, -0.03, 0.055]]) sc.limb(add(pc, [s * dx, 0.03, dz]), add(pc, [s * (dx + 0.045), 0.03 + len, dz * 1.5]), 0.013, 0.0015, { mat: "gold", bone: "chest", k: 0.002, vdil: 0.55 });
    ...
    const sk = sc.add(S.custom((x, y, z) => {
      ...
      return Math.max(Math.hypot(x, z / szs) - r - 0.005 * u * Math.sin(a * 6 + 0.6), y, -y - h);
    ...
    sc.paint(S.custom((x, y, z) => Math.abs(y - 0.015) - 0.0065, [-1, -1, -1, 1, 1, 1]), { mat: "gold", soft: 0.001, only: ["robe", "blue"] });
    ...
    sc.paint(S.custom((x, y, z) => y - capeBot - 0.014, capeB), { mat: "gold", soft: 0.001, only: ["cape", "lining"] });
    sc.part = "body";
