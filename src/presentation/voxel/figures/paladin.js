/* 圣光裁决者 — Light Arbiter (card paladin): a paladin in white-and-gold plate — long golden hair under a spiked gold
 * sun crown, a gold sun on the breastplate, broad gold-rimmed pauldrons, a floor-length white surcoat skirt edged in
 * gold and a long white cape — a tall white kite shield with a gold sun on the left arm and, the signature at board
 * scale, a greatsword with a broad gold cross-guard, held point-down at rest as on the card. The attack raises the
 * greatsword up in front and over the head and brings it down in a heavy vertical cleave. */
EmberVoxelKit.define("paladin", (() => {
  ...
  const B0 = 0.05, BL = 0.5;
  function greatsword() {
    const sc = new Sculpture(), PX = EmberVoxelKit.pixel;   // pixel: a thick blade, no fuller or guard tips
    ...
    const o = { bone: "p" };
    sc.limb([0, -0.085, 0], [0, 0.03, 0], 0.0095, 0.0095, { ...o, mat: "grip", k: 0.002 });
    sc.add(S.ell(0.016, 0.02, 0.012), { ...o, mat: "gold", p: [0, -0.1, 0], k: 0.002 });
    // cross-guard: a broad bar sweeping up at the ends, a sun boss in the middle
    for (const s of [1, -1]) sc.limb([0, 0.036, 0], [s * 0.085, 0.052, 0], 0.011, 0.007, { ...o, mat: "gold", k: 0.003, sz: 0.7 });
    if (!PX) for (const s of [1, -1]) sc.limb([s * 0.08, 0.05, 0], [s * 0.1, 0.075, 0], 0.008, 0.003, { ...o, mat: "gold", k: 0.002, vdil: 0.5 });
    ...
    const blade = S.custom((x, y, z) => {
      ...
      const dz = Math.abs(z) - (PX ? 0.011 : 0.0065 * (1 - 0.6 * Math.min(1, Math.abs(x) / w)) + 0.0015);
  ...
  // ------------------------------------------------------------ figure
  function build(fam) {
    const P = FAM[fam], sc = new Sculpture(), q = 1.25, PX = K.pixel;   // PX: pixel sprite — fat locks, flat plates, calm cloth
    humanoidBones(sc, P);
    ...
    mats(sc, {
      ...
      hair: { c: PX ? 0xb87a44 : 0xf0dca6, rough: 0.6, cls: CLS.hair, vary: 0.08 },
      mail: { c: 0x6a6878, rough: 0.5, metal: 0.5, cls: CLS.metal, vary: 0.04 },
    ...
    for (const s of [1, -1]) {
      ...
      if (!PX) sc.add(S.ell(0.07, 0.012, 0.012), { mat: "gold", p: add(pc, [0, 0.06, 0]), r: [0, 0, s * 0.35], bone: "chest", k: 0.002 });
    }
    ...
    sc.paint(S.custom((x, y, z) => (z < 0.04 ? 1 : (PX ? sun2(x, y - P.chest[0] - 0.008, 0.026, 0.064, 6, 0.6) : sun2(x, y - P.chest[0] - 0.008, 0.02, 0.06, 8, 0.5))), [-0.2, 0.5, -0.2, 0.2, 0.9, 0.2]), { mat: "gold", soft: 0.001, only: ["plate"] });
    ...
    if (!PX) sc.add(S.cyl(0.005, 0.02, 0.002), { mat: "gold", p: [0, P.pelvis[0] + 0.023, P.pelvis[3] + 0.04], r: [Math.PI / 2, 0, 0], bone: "root", k: 0.001 });
    ...
    if (PX) for (const u of [-1, 0, 1]) {   // pixel: three fat locks; the middle one stops above the eye band
      const root = [0.012 * u, HC[1] + HR[1] * 0.6, HC[2] + HR[2] * 0.74];
      const mid = [HR[0] * 0.45 * u, ey + 0.048, P.faceZ + 0.004 * q];
      const tip = [HR[0] * 0.8 * u, ey + 0.03 - 0.034 * Math.abs(u), P.faceZ - 0.016 * q * Math.abs(u)];
    ...
    for (const s of [1, -1]) {
      ...
      const sp = strand(sc, [root, a, lerp(a, tip, 0.5), tip], (PX ? 0.02 : 0.016) * q, (PX ? 0.007 : 0.004) * q, { k: 0.01, wg: 4, wave: PX ? 0 : 0.005, wd: [s, 0, 0], grooves: 0.0012 });
      sp.wfn = (x, y) => { const t = sstep(P.chinY, P.chest[0], y); return [["head", 1 - t * 0.7], ["chest", t * 0.7]]; };
    }
    const nB = PX ? 3 : 6;   // pixel: three broad locks down the back, no waves
    for (let i = 0; i < nB; i++) {
      const u = (i / (nB - 1)) * 2 - 1, az = Math.PI + u * 1.25;
      ...
      const sp = strand(sc, [root, out, mid, tip], PX ? 0.04 - 0.006 * Math.abs(u) : 0.024 - 0.004 * Math.abs(u), PX ? 0.012 : 0.005, { wave: PX ? 0 : 0.007, wd: [Math.cos(az), 0, 0], waves: 2, phase: i * 1.7, k: 0.012, wg: 5, swell: 0.2, bone: "hairB1", grooves: 0.0014 });
    ...
    // (pixel: the circlet only round the front, so it never pokes through the hair fall behind)
    sc.add(S.custom((x, y, z) => Math.max(Math.abs(Math.hypot(x / crx, (z - cz) / crz) - 1) * crx - 0.006, Math.abs(y - cy) - (PX ? 0.012 : 0.008), PX ? cz - z : -1), pad([-crx, cy - 0.02, cz - crz, crx, cy + 0.02, cz + crz])),
      { mat: "gold", bone: "head", k: 0.002 });
    for (const az of PX ? [0, 0.6, -0.6] : [0, 0.4, -0.4, 0.8, -0.8, 1.2, -1.2]) {   // pixel: three big spikes
      const len = PX ? (az === 0 ? 0.09 : 0.065) : 0.075 - 0.035 * Math.abs(az) / 1.2 + (az === 0 ? 0.02 : 0);
      const base = [crx * Math.sin(az), cy + 0.004, cz + crz * Math.cos(az)];
      const dir = norm([Math.sin(az) * 0.35, 1, Math.cos(az) * 0.3]);
      sc.limb(base, add(base, mul(dir, len)), PX ? 0.018 : 0.011, PX ? 0.004 : 0.0015, { mat: "gold", bone: "head", k: 0.002, vdil: 0.55, sz: PX ? 0.8 : 0.6 });
    }
    ...
    sc.paint(S.custom((x, y, z) => Math.abs(y - y1 - (PX ? 0.011 : 0.007)) - (PX ? 0.011 : 0.0065), [-1, 0, -1, 1, 0.3, 1]), { mat: "goldC", soft: 0.001, only: ["cloth"] });
    sc.paint(S.custom((x, y, z) => (z < 0.03 ? 1 : (PX ? Math.abs(Math.abs(x) - 0.03) - 0.011 : Math.abs(Math.abs(x) - 0.03) - 0.0065)), [-1, -1, -1, 1, 1, 1]), { mat: "goldC", soft: 0.001, only: ["cloth"] });
    ...
    const ch = capeTop - capeBot, capeZc = (x, y) => {
      const u = clamp((capeTop - y) / ch, 0, 1), w = mix(0.085, 0.23, Math.pow(u, 0.75));
      return { u, w, zc: mix(capeZ, capeZ - 0.1, u) - (PX ? 0.006 : 0.018) * Math.sin((x / w) * (PX ? 2 : 5) * 1.57 + 0.5) * (0.25 + u) + (0.55 - 0.25 * u) * (x * x) / w };
    };
    const hem = (x) => (PX ? 0.008 : 0.012 * (1 + Math.sin(x * 30 + 0.6)));   // pixel: a straight hem
    ...
    if (!PX) for (const s of [1, -1]) sc.add(S.cyl(0.005, 0.015, 0.002), { mat: "gold", p: [s * 0.064, capeTop - 0.014, capeZc(s * 0.064, capeTop - 0.014).zc - 0.009], r: [Math.PI / 2, 0, 0], bone: "chest", k: 0.001, part: "cloth" });
    sc.part = "body";

    // ---- kite shield on its own bone (local x across, y up, z out of the face): white field, gold rim, gold sun
    const W = 0.13, yT = 0.24, yB = -0.3, H = yT - yB, th = V, zf = 4 * V, bul = PX ? 0 : 0.02, rimW = PX ? 0.022 : 0.016;
    ...
    sc.paint(S.custom((x, y, z) => (far(x, y, z) ? 1 : Math.max(PX ? sun2(x, y - 0.02, 0.036, 0.1, 6, 0.6) : sun2(x, y - 0.02, 0.03, 0.11, 8, 0.5), zc(x) - z, outl(x, y) + rimW)), SB), { mat: "gold", p: G, soft: 0.001, only: ["field"] });
  ...
  /* the greatsword swings in the plane of up U and forward F: angle φ from U toward F, the hand on a circle round a
  ...
  function pose(fig, clip, t, T, C) {
    ...
    const U = V3(-0.12, 1, 0).normalize(), F = V3(0.22, 0, 1).normalize();
