...
EmberVoxelKit.define("paladin", (() => {
  ...
  function greatsword() {
    ...
    for (const s of [1, -1]) sc.limb([s * 0.08, 0.05, 0], [s * 0.1, 0.075, 0], 0.008, 0.003, { ...o, mat: "gold", k: 0.002, vdil: 0.5 });
  ...
  function build(fam) {
    ...
    const W = 0.13, yT = 0.24, yB = -0.3, H = yT - yB, th = V, zf = 4 * V, bul = 0.02, rimW = 0.016;
    ...
    sc.paint(S.custom((x, y, z) => (far(x, y, z) ? 1 : Math.max(sun2(x, y - 0.02, 0.03, 0.11, 8, 0.5), zc(x) - z, outl(x, y) + rimW)), SB), { mat: "gold", p: G, soft: 0.001, only: ["field"] });
