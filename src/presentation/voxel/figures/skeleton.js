...
EmberVoxelKit.define("skeleton", (() => {
  ...
  function boneSpear() {
    ...
    const R = 0.0095, top = 0.5, bot = -0.44;
  ...
  function build(fam) {
    ...
    const B = { mat: "bone" }, thin = { ...B, vdil: 0.55 };
    ...
    for (const s of [1, -1]) sc.add(ember, { mat: "eyeGlow", p: [s * 2.5 * V, row(70), 0.0563], bone: "head", k: 0.001, cs: 0.01, vdil: 0.6 });
    ...
    sc.limb([0, 0.738, -0.03], [0, 0.842, -0.012], 0.011, 0.01, { ...B, bone: "neck", k: 0.004, vdil: 0.5 });
    for (const y of [0.77, 0.795, 0.82]) sc.add(S.ell(0.017, 0.006, 0.014), { ...B, p: [0, y, -0.022], bone: "neck", k: 0.002 });
    sc.limb([0, 0.585, -0.04], [0, 0.745, -0.042], 0.012, 0.011, { ...B, bone: "chest", k: 0.004 });
    for (const y of [0.61, 0.645, 0.68, 0.715]) sc.add(S.box(0.005, 0.006, 0.011), { mat: "boneD", p: [0, y, -0.055], bone: "chest", k: 0.002 });
    ...
    for (const y of [0.53, 0.555, 0.58]) sc.add(S.ell(0.019, 0.008, 0.017), { ...B, p: [0, y, -0.03], bone: "spine", k: 0.002 });
    ...
    for (const s of [1, -1]) {
      ...
      sc.limb(a.S, a.E, 0.0095, 0.008, { ...thin, bone: "arm" + n, k: 0.003 });
      sc.add(S.ell(0.016, 0.015, 0.014), { ...B, p: a.E, bone: "fore" + n, k: 0.003 });
      sc.limb(a.E, a.W, 0.0085, 0.0075, { ...thin, bone: "fore" + n, k: 0.003 });
      hand(sc, P, s, a, "bone");
      sc.add(S.cyl(0.014, 0.0185, 0.003), { mat: "iron", p: lerp(a.E, a.W, 0.8), R: K.rotY2(...a.dir), bone: "fore" + n, k: 0.002 });
      sc.add(S.cyl(0.004, 0.02, 0.002), { mat: "leather", p: lerp(a.E, a.W, 0.8), R: K.rotY2(...a.dir), bone: "fore" + n, k: 0.001 });
      sc.add(S.sphere(0.017), { ...B, p: add(l.H, [s * 0.006, 0.004, 0]), bone: "thigh" + n, k: 0.002 });
      sc.limb(l.H, l.K, 0.0115, 0.0095, { ...thin, bone: "thigh" + n, k: 0.003 });
      ...
      sc.limb(l.K, l.A, 0.0105, 0.008, { ...thin, bone: "shin" + n, k: 0.003 });
      sc.add(S.cyl(0.015, 0.021, 0.003), { mat: "iron", p: add(l.A, [0, 0.022, 0]), bone: "shin" + n, k: 0.002 });
      sc.add(S.cyl(0.0035, 0.0225, 0.002), { mat: "ironD", p: add(l.A, [0, 0.03, 0]), bone: "shin" + n, k: 0.001 });
    ...
    sc.add(S.cyl(0.004, 0.014, 0.002), { mat: "iron", p: [0.012, 0.5, 0.054], r: [Math.PI / 2, 0, 0], bone: "root", k: 0.001 });
    // ---- cloth: torn mantle (low over the left shoulder), torn cape, ragged loincloth
    sc.part = "cloth";
    // bunched collar round the neck
    sc.add(S.custom((x, y, z) => Math.hypot(Math.hypot(x, (z + 0.012) / 0.9) - 0.054 - 0.004 * Math.sin(Math.atan2(x, z) * 5), (y - 0.764) * 1.25) - 0.019, pad([-0.08, 0.74, -0.09, 0.08, 0.79, 0.07])),
    ...
    const drape = (x, y, z) => {
      const dY = 0.73 - 0.012 * clamp((x - 0.08) / 0.08, 0, 1), fold = 0.006 * Math.sin(x * 80 + 0.5) * sstep(dY, dY - 0.05, y);
      ...
      const hem = z > dC ? 0.7 - 0.05 * sstep(0.1, 0.17, x) + 0.03 * tear(x * 34 + 2) : 0.6;
      return Math.max(d - 0.0065, hem - y, 0.078 + 0.016 * tear(y * 40 + 3) - x, x - 0.172 - 0.012 * tear(y * 45 + 1));
    };
    ...
    const capeZc = (x, y) => {
      ...
      return { u, w, zc: mix(capeZ, capeZ - 0.075, u) - 0.014 * Math.sin((x / w) * 5 * 1.57 + 0.9) * (0.25 + u) + (x > 0 ? 0.85 - 0.25 * u : 0.5 - 0.25 * u) * (x * x) / w };
    };
    const capeF = (x, y, z) => {
      const c = capeZc(x, y), hem = capeBot + 0.05 * sstep(0.15, -0.15, x) + 0.075 * tear(x * 26 + 3);
      ...
      return smax(Math.abs(z - c.zc) - 0.006, Math.max(Math.abs(x) - c.w, y - capeTop, hem - y, -rent * 0.01), 0.005);
    };
    ...
    const flap = (zf, dz, yb, seed) => (x, y, z) => {
      const u = clamp((0.505 - y) / (0.505 - yb), 0, 1), w = mix(0.056, 0.07, u), zc = zf + dz * u + 0.004 * u * Math.sin(x * 60 + seed);
      const hem = yb + 0.045 * tear(x * 32 + seed);
    ...
    sc.paint(S.custom((x, y, z) => (Math.abs(z) < 0.03 || y > 0.46 ? 1 : y - 0.34 - 0.03 * tear(x * 32 + 4)), [-0.08, 0.3, -0.08, 0.08, 0.5, 0.08]), { mat: "ragD", p: [0, 0, 0], soft: 0.001, only: ["rag"] });
    sc.part = "body";
    return {
      sc, P, kind: "humanoid",
