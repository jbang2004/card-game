...
EmberVoxelKit.define("treant", (() => {
  ...
  function treant() {
    ...
    for (const [x0, x1, y0, y1] of [[-7.5, -3, 22, 46], [8, 5, 23, 44], [1, 6.5, 30, 50]]) {
      const pts = [];
    ...
    for (const s of [1, -1]) sc.paint(S.box(V(1), V(0.5), V(1.5)), { mat: "eye", p: [s * V(3), V(64.5), V(6.8)], soft: 0.001 });
    ...
    for (const s of [1, -1]) {
      ...
      for (const [dx, dz, len] of [[0.15, 1, 11], [0.8, 0.6, 9], [1, -0.15, 8], [0.45, -0.9, 8], [-0.45, 0.9, 8]]) {
        const d = norm([s * dx, 0, dz]), p0 = [l.A[0] + d[0] * V(3), V(4), l.A[2] + d[2] * V(3)];
        sc.add(S.chain([[...p0, V(2.8)], [p0[0] + d[0] * V(len * 0.5), V(2.4), p0[2] + d[2] * V(len * 0.5), V(2)], [p0[0] + d[0] * V(len), V(1.4), p0[2] + d[2] * V(len), V(1.2)]]),
    ...
    sc.inPart("hair", () => {
      for (let i = 0; i < 7; i++) {
        const u = i / 6 - 0.5, top = [u * V(10), V(60) - Math.abs(u) * V(1.5), V(8.8) - Math.abs(u) * V(2.5)];
        const mid = [u * V(9.5), V(52), V(11)], tip = [u * V(4), V(42.5) + Math.abs(u) * V(9), V(11.2)];
        const b = sc.add(S.bez(top, mid, tip, V(1.8), V(0.9)), { mat: i % 2 ? "beard" : "beardL", bone: "head", k: 0.005, cs: 0.03 });
      ...
      for (const [x, y, z, r, m] of C) for (const s of x ? [1, -1] : [1]) cl([s * V(x) + (s < 0 ? V(0.6) : 0), V(y + (s < 0 ? 0.7 : 0)), V(z)], V(r), s < 0 && m === "leafL" ? "leaf" : m);
    });
