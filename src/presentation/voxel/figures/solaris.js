...
EmberVoxelKit.define("solaris", (() => {
  ...
  function sunsword() {
    ...
    if (!PX) for (const s of [1, -1]) sc.limb([s * 0.066, 0.048, 0], [s * 0.084, 0.07, 0], 0.008, 0.003, { ...o, mat: "gold", k: 0.002, vdil: 0.5 });
  ...
  function build(fam) {
    ...
    mats(sc, {
      ...
      hair: { c: PX ? 0xffd86a : 0xf0dca6, rough: 0.6, cls: CLS.hair, vary: 0.08 },
      mail: { c: 0x2c3c7c, rough: 0.8, cls: CLS.cloth, vary: 0.05 },
      halo: { c: PX ? 0xf06a10 : 0xf4c85a, rough: 0.3, emit: PX ? 0.45 : 1.1, cls: CLS.glow },   // pixel: a hot orange ring, apart from her golden hair haloL: { c: 0xfff0b8, rough: 0.3, emit: 1.6, cls: CLS.glow },
    ...
    if (!PX) sc.add(S.custom((x, y, z) => Math.max(Math.abs(Math.hypot(x / crx, (z - cz) / crz) - 1) * crx - 0.006, Math.abs(y - cy) - 0.008), pad([-crx, cy - 0.02, cz - crz, crx, cy + 0.02, cz + crz])),
      { mat: "gold", bone: "head", k: 0.002 });
    if (!PX) for (const az of [0, 0.4, -0.4, 0.8, -0.8, 1.2, -1.2]) {   // pixel: no crown, the halo alone crowns her
    ...
    const Rh = 0.13, HS = PX ? 1.3 : 1, HP = PX ? add(HALO, [0, 0, -0.04]) : HALO;   // pixel: a bigger halo, clear of the enlarged head
    const NR = PX ? 8 : 12;   // pixel: eight broad rays
    const rays = (x, y) => { const r = Math.hypot(x, y), a = Math.atan2(y, x), f = Math.abs(((((a / (2 * Math.PI)) * NR) % 1) + 1) % 1 - 0.5) * 2; // 1 on a ray's axis
      const long = Math.floor(((a / (2 * Math.PI)) * NR + NR + 0.5)) % 2 ? 0.1 : 0.07;
      return Math.max(r - Rh - 0.01 - long * (PX ? Math.max(0, (f - 0.3) / 0.7) : Math.pow(Math.max(0, (f - 0.55) / 0.45), 0.7)), Rh - r); };
    sc.add(S.custom((x, y, z) => Math.max(Math.min(Math.abs(Math.hypot(x, y) - Rh) - (PX ? 0.02 : 0.016), rays(x, y)), Math.abs(z) - (PX ? 0.009 : 0.005)), pad([-Rh - 0.12, -Rh - 0.12, -0.01, Rh + 0.12, Rh + 0.12, 0.01], 0.03)),
      { mat: "halo", p: HP, s: HS, bone: "halo", k: 0.001, vdil: 0.4 });
    sc.paint(S.custom((x, y, z) => Math.abs(Math.hypot(x, y) - Rh + (PX ? 0.006 : 0.008)) - (PX ? 0.01 : 0.0065), pad([-Rh, -Rh, -0.02, Rh, Rh, 0.02])), { mat: "haloL", p: HP, s: HS, soft: 0.001, only: ["halo"] });
    ...
    const W = 0.13, yT = 0.2, yB = -0.24, H = yT - yB, th = V, zf = 4 * V, bul = PX ? 0 : 0.02, rimW = PX ? 0.022 : 0.016;
    const hw = PX ? (y) => W * Math.sqrt(Math.max(0, 1 - Math.pow(clamp((0.02 - y) / (0.02 - yB), 0, 1), 2)))   // pixel: a heater (flat top, round foot)
      : (y) => W * Math.pow(Math.max(0, 1 - Math.pow(clamp((yT - y) / H, 0, 1), 2.1)), 0.62);
    ...
    sc.paint(S.custom((x, y, z) => (far(x, y, z) ? 1 : Math.max(PX ? sun2(x, y, 0.036, 0.1, 6, 0.6) : sun2(x, y - 0.0, 0.03, 0.105, 8, 0.5), zc(x) - z, outl(x, y) + rimW)), SB), { mat: "gold", p: G, soft: 0.001, only: ["field"] });
