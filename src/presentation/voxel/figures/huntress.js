...
EmberVoxelKit.define("huntress", (() => {
  ...
  function huntress(fam) {
    ...
    for (const dy of [-0.022, -0.004, 0.014]) wrap(sc, RG.box(-X0, X0, P.waist[0] + dy * q - 0.0025 * q, P.waist[0] + dy * q + 0.0025 * q), "strap", 0.0035 * q);
    wrap(sc, RG.band([0, P.pelvis[0] + P.pelvis[2] * 0.25, 0], [0, 1, -0.25], 0.006 * q, RG.box(-0.2, 0.2, P.pelvis[0] - 0.05, P.waist[0])), "strap", 0.0045 * q);
    // skirt
    sc.part = "cloth";
    skirt(sc, P, P.pelvis[0] + P.pelvis[2] * 0.3, mix(P.hipY, P.kneeY, chib ? 0.3 : 0.4), P.pelvis[1] * 1.14 + 0.006, P.pelvis[1] * (chib ? 1.4 : 1.36), "tunic", { folds: 7, amp: chib ? 0.01 : 0.008, sz: 0.86, t: 0.006 * (chib ? 1.4 : 1) });
    sc.part = "body";
    ...
    for (const s of [1, -1]) {
      ...
      for (const u of [0.14, 0.5, 0.86]) wrap(sc, RG.seg(lerp(a.E, a.W, u - 0.025), lerp(a.E, a.W, u + 0.025), 0.07), "strap", 0.0025 * q);
      wrap(sc, RG.ball(add(a.W, mul(a.dir, 0.035 * P.hand)), 0.05 * P.hand), "glove", 0.002 * q);
    }
    // cape + capelet over the shoulders
    sc.part = "cloth";
    cape(sc, P, capeTop, capeBot, P.shX * 0.95, P.shX * (chib ? 1.5 : 1.9), "cape", { folds: 6, amp: chib ? 0.012 : 0.01, t: chib ? 0.009 : 0.0065 });
    bell(sc, P.neck[0] + 0.012 * q, P.chest[0] - P.chest[2] * 0.35, P.neck[2] * 1.9, P.shX + P.delt * 1.15, "cape", { t: chib ? 0.01 : 0.0065, sz: (P.chest[3] * 1.5) / (P.shX + P.delt), folds: 9, amp: 0.006 * q, z: -0.006, bones: [["chest", 1]] });
    ...
    const faceOpen = S.at(S.ell(P.face[0] * (chib ? 1.2 : 1.45), (P.crown - P.chinY) * (chib ? 0.5 : 0.62), 0.12 * (chib ? 2 : 1)), 0, (P.eyeY - hc[1]) - 0.004, (P.faceZ - hc[2]) + 0.1 * (chib ? 1.6 : 1) - (chib ? 0.06 : 0.035));
    ...
    sc.add(hood, { mat: "hood", p: hc, bone: "head", k: 0.006, cs: 0.03, wfn: null });
    ...
    sc.add(S.cyl(0.003 * q, 0.009 * q, 0.002), { mat: "gold", p: [0, P.shY - 0.02 * q, P.chest[3] + 0.024 * q], r: [Math.PI / 2 - 0.3, 0, 0], bone: "chest", k: 0.002 });
    // hair: cap under the hood, side-swept fringe, long wavy locks over the shoulders
    sc.part = "hair";
    huntressHair(sc, P, fam);
    sc.part = "body";
    sc.faceKind = "huntress";
    return { sc, P, kind: "humanoid", props: [{ sc: spearProp(fam), bone: "handR", at: armJoints(P, -1).W, grip: "spear" }] };
  }
