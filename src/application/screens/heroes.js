...
const EmberHeroScreens = (() => {
  function create(context) {
    ...
    function showHeroes() {
      ...
      const updateContractSlots = () => {
        const count = document.querySelectorAll(
          "[data-contract]:checked",
        ).length;
        document.querySelector(".contract-slot-preview").innerHTML =
          Array.from(
            { length: 3 },
            (_, i) => `<i class="${i < count ? "filled" : ""}"></i>`,
          ).join("") + `<b>${count} / 3</b>`;
      };
      document
        .querySelectorAll("[data-contract]")
        .forEach((el) => el.addEventListener("change", updateContractSlots));
      const plan = () => {
      ...
      const updateModeCards = () =>
        modeCards
          ...
          .forEach((b) =>
            b.setAttribute(
              "aria-pressed",
              String(b.dataset.mode === modePicker.value),
            ),
          );
      ...
      modeCards.querySelectorAll("button").forEach(
        (b) =>
          (b.onclick = () => {
            modePicker.value = b.dataset.mode;
            modePicker.dispatchEvent(new Event("change", { bubbles: true }));
          }),
      );
