...
可离线运行的单人策略卡牌游戏：48 张可组牌卡、8 张衍生牌、3 位英雄、5 场首领战役。桌面与手机使用同一 Canvas 酒馆场景，手机保留独立横竖屏布局与触屏操作。
...
保留 56 张已确认卡图、v1 存档及原四个存储键。现有 CSS 层级仍为 `legacy → layout → theme → components`，其中基础布局规则仍被当前 UI 使用，不因历史命名删除有效样式。
