...
桌面与手机使用同一套手势：**点击任何手牌都会完整升起供阅读**，包括法力不足、场上已满或对方回合不能出的牌。可出的牌点击后进入准备状态，再点战场空位或合法目标确认；也可以从展开的牌继续拖动出牌。不能出的牌只显示原因，不进入出牌状态。再点展开的牌、顶部“取消”、Esc 或右键手牌可收起。手机手牌原生横滑、向上拖放和长按扶起仍保留，桌面悬停预览仍可用。操作说明和通知统一位于顶栏；竖屏在导航下预留一行，横屏和大屏使用中央状态槽。实现见 [触屏出牌手势](docs/design/HAND_GESTURES.md)，验证见 [手牌展开与顶栏改进](docs/qa/HAND_READING_20260919.md)。
