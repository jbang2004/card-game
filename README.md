...
手牌只有一套手势，桌面与手机一致：**拖到战场松手出牌**，需要目标的牌在拖动中实时亮起合法目标并拉出瞄准线，**松在合法目标上结算、松回手牌或非法位置取消且不消耗法力**，横向滑动始终用于浏览手牌。**点按**手牌等于快捷出牌：无目标牌直接打出，有目标牌进入瞄准（再点目标确认，右键 / Esc / 取消按钮退出）。**查看**是独立的详情层：桌面悬停或右键、手机长按任意卡牌，都会把该卡放大到屏幕中央（无面板外框），点击任意位置收起；手机不再有"先弹详情卡再点按钮出牌"的中间步骤。实现与验收见 [触屏出牌手势](docs/design/HAND_GESTURES.md)。
