...
test("every selectable hero has live artwork; every entry is a hero or a card", () => {
  for (const hero of campaign.heroes) assert.ok(maps[hero.portraitId], hero.portraitId);
  const known = new Set([...campaign.heroes.map((h) => h.portraitId), ...cards.map((c) => c.id)]);
  for (const id of Object.keys(maps)) assert.ok(known.has(id), id);
  assert.deepEqual(Object.keys(rigs).sort(), Object.keys(maps).sort(), "Run tools/bake_live_art.py.");
  const referenced = [];
  for (const [id, set] of Object.entries(maps)) {
    ...
    for (const kind of KINDS) {
      ...
      assert.ok(fs.statSync(path.join(root, "art/live-art", `${id}-${kind}.webp`)).size > 1000);
...
test("rigs are complete: a pair of blinking eyes (or none) and a displacement body", () => {
...
test("live portrait is presentation-only and registered ahead of the hero screen", () => {
  ...
  assert.ok(at("LIVE_ART_MAPS") < at("LIVE_ART_RIGS") && at("LIVE_ART_RIGS") < at("LIVE_ART"));
