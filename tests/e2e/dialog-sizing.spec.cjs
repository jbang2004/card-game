...
test("desktop toolbar SVGs remain aligned inside visible controls", async ({
  page,
}) => {
  await page.goto("./?debug=1");
  await page.waitForFunction(() => window.Emberfall && !AtelierWorld.loading);
  ...
  for (const icon of metrics) {
    ...
    if (icon.id === "sound-btn")
      ...
      expect(Math.abs(icon.offset[0]) + 9).toBeLessThanOrEqual(
        icon.button[0] / 2,
      );
...
test("long-form dialog exposes one vertical scroll surface", async ({
  page,
}) => {
  await page.goto("./?debug=1");
  await page.waitForFunction(() => window.Emberfall && !AtelierWorld.loading);
  await page.evaluate(() => Emberfall.showHelp());
  await expect(page.locator("#modal .folio-dialog > .folio-pane")).toHaveCount(
    1,
  );
