...
// The ranger's live miniature stands on the player hero plate, follows the
// effect layer's cues and never changes the rules state.
test("ranger miniature appears on the hero plate and answers power and damage", async ({ page }) => {
  const errors = [];
  ...
  await page.waitForFunction(() => EmberVesperHero.diagnostics().status === "ready");
  const box = await page.locator("#vesper-miniature").boundingBox();
  ...
  expect(await page.evaluate(() => getComputedStyle(document.getElementById("vesper-miniature")).pointerEvents)).toBe("none");
  await page.evaluate(() => { const g = EmberDebug.game; g.s.p.mana = 10; Emberfall.renderNow(); Emberfall.usePower(); });
  await page.waitForFunction(() => EmberVesperHero.diagnostics().cues.some((c) => c.kind === "shot"));
  ...
  await page.waitForFunction(() => EmberVesperHero.diagnostics().cues.some((c) => c.kind === "hurt"));
