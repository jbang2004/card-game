...
test('original flame, renderer and rule inputs remain byte-identical after semantic improvements',()=>{const j=JSON.parse(fs.readFileSync('docs/qa/remaster-r9-baseline.json'));for(const [p,h] of Object.entries(j).filter(([p])=>!p.includes("benchmark-")))assert.equal(crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex'),h,p);});
