...
test('lifecycle work preserves original flame, renderer and rule baseline',()=>{const h=JSON.parse(fs.readFileSync('docs/qa/remaster-r9-baseline.json'));for(const [p,sha]of Object.entries(h).filter(([p])=>!p.includes("benchmark-")))assert.equal(crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex'),sha,p);});
