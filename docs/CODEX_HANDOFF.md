...
> 历史 v0.7 输入交接记录。当前 v0.11 开发入口是 [README](../README.md)、[架构](ARCHITECTURE.md) 和 [素材说明](ASSETS.md)。下文提到的旧渲染器、缓存和 Python 回归已退役，不要据此恢复兼容实现。
...
本轮视觉/UI 调整详见 `docs/UI_POLISH_20260909.md`。收藏将英雄、预设与契约配置置于“调整套牌”折叠区，浏览器测试须像玩家一样先展开（`tests/e2e/helpers/deck-tools.cjs`）。英雄、设置、地图、手册、奖励、起手与契约弹窗通过 `.modal-scroll` 保留固定标题/操作区；增强器插入设置项须进入该滚动容器。手机契约入口位置由 `mobile-view.js` 统一计算。普通构建新增标准库 UI 素材生成器，不依赖在线服务。未发布线上。

### 专用资产与细节复审

见 `docs/UI_CRAFT_REVIEW_20260909.md`。英雄选择保持焦点/滚动，勾选不受拱形裁切；地图清除旧固定宽度规则；result/touch-card/touch-hand/inspect进入统一滚动容器。手机牌组配置打开时独占中部，`.deck-tools-body`独立滚动，横屏隐藏重复标题保留页签；错误采用可展开摘要。不要重新依赖原生details匿名包装层参与flex排版。瞬时战斗提示的modal判定覆盖body，避免移动端重挂载后的穿透。所有细节CSS已合入components，代理临时CSS文件已移除。新增4份细节浏览器测试。
