...
2026-09-24 用户在六种风格选型后确认体素风（参照 voxel-musou）。2026-09-25 用户认为体素和精致的战场不协调，看过彩绘手办、金属棋子、细体素三种方案后，选定 **Q 版像素风**（参照 Q 版像素 RPG 精灵）：同一批代码雕刻的 3D 角色，渲染成像素精灵。动作、打击感、站位与朝向不变，只换画法。本文件是约定，改数值先改本文件。体素管线（voxelize、`bakeVoxel`、体素材质、`pixelFace`）保留作参照与测试工具，战场不再使用。

## 1. 风格规格（所有角色共用）

- **生成（`EmberVoxelKit.bakePixel`，后台线程）**：角色以像素版构建（`EmberVoxelKit.pixel = true`，见下），按部件用 surface nets 取面：身体 0.008、头发 0.006、布料 0.0048、其他 0.006、道具 0.004。不做表面位移（`plain`），不做材质色差和纹样（`flat`）。之后逐部件用 `EmberMeshSimplify` 减到 7%（道具 18%），保底 260 面（道具 120 面），细环、细刃不会被吃掉。减面代价计入颜色差和发光差，颜色边界和发光纹不会被拉花。最后做保边颜色平滑（容差 0.07），并平滑 AO、曲率和法线，三阶色调的分界线因此干净。碎块和拼合用的点从简化后的顶点里均匀取约 900 个。一个人形约 1–2 秒、1–2 万面；动物 0.1–0.3 秒、1.4–3 千面。
- **像素版造型（每个角色文件在 `build()` 里读 `EmberVoxelKit.pixel`）**：
  - 去掉毛皮、树皮、破布上的随机噪点，破边改成几个规整的大齿。
  - 细发束、羽毛、毛刺合并成少数几块大造型；小于约 0.02 的细节要么去掉、要么放大。
  - 标志物放大加粗：武器、王冠、光环、翅膀、鹿角。薄板至少 0.02 厚。
  - 刘海、兜帽、头盔不遮住眼睛带（`P.eyeY ± 0.015`，横向 `±1.6 × P.eyeX`）。
  - 相邻色块要拉开颜色差。徽记和光环边缘做成凸起的几何体，不能只靠涂色。
  - 像素版关闭时，角色必须和原来逐字节相同。
- **比例**：人形用 `chunky` 体型族；像素精灵的头骨放大 1.55 倍（兽类 1.25 倍），得到 Q 版头身比例。
- **着色（`EmberVoxelRender.spriteMaterial`）**：三阶色调——亮面、中间调、冷紫暗面。脸部区域固定用亮面，不让阴影线切过脸。金属只有一道硬高光；饱和度 ×1.22；发光部位用材质的 `emit`；受击光在接触帧全白，之后按颜色渐退。
- **脸（`spriteFace`）**：五官画在角色自己的像素格上（一格约等于头上一个屏幕像素，`texel` 0.0105），用最近邻采样。
  - 人形：眼睛 2×3，依次是睫毛行、白色高光、虹膜；眉毛 1 像素，嘴 1–2 像素。
  - 兽类：眼睛 2×2，带一个白色高光。
  - 表情有 `open / closed / fierce / hurt / focus`，睡眠时闭眼。
- **像素画（`EmberPixelPass`）**：角色层按 CSS 尺寸的 0.5 倍画进渲染目标，不做抗锯齿，再经一道着色器画到画布上，由 `image-rendering: pixelated` 放大：
  - 每个实心轮廓外圈加一像素墨色（#1c1422）描边；
  - 内部强色差交界处较暗的一侧压暗，即选择性内描边；
  - 显示色限定在 18 级；
  - 光晕和粒子原样通过，不描边。
- **分层**：战场的立体角色层（`#miniature-stage`）、英雄头像窗（`.hero-figure-canvas`）和角色馆的角色层都经这道像素画处理。角色馆的天空和地面在后面一层清晰绘制。
- **轮廓**：每个角色至少有一个在棋盘上能认出的特征，比如武器、角、翅膀、兜帽、盾牌或一块醒目的配色。
...
presentation/voxel/
  ...
  voxelize.js    EmberVoxel    体素化（旧体素风，保留作参照）
  simplify.js    EmberMeshSimplify  QEM 半边折叠减面（保颜色、发光、脸、骨骼边界）、保边颜色平滑、属性平滑
  kit.js         EmberVoxelKit 体型族、人形身体、服装与发束工具、道具、四足骨架、角色注册表；
                               bakePixel（战场用）/ bakeVoxel（旧）；pixel 标记让角色构建像素版
  figures/*.js                 每个角色一个文件：EmberVoxelKit.define(id, spec)
  render.js      EmberVoxelRender  烘焙缓存、蒙皮网格、像素精灵材质与像素五官、Q 版头部（旧体素材质保留）
  pixelpass.js   EmberPixelPass    像素画：低分辨率渲染目标 → 外描边 + 选择性内描边 + 限色
...
tools/voxel-gallery/               像素角色馆：同一个 arena 驱动的独立页面（全员 / 单人 / 对战，带合成音效）
```

- 三维库只有 `src/vendor/vesper-three.js`（全局 `EmberVesperThree`）一份。
- 像素精灵在运行时生成，每个角色约 0.1–2 秒，按角色缓存，**在 worker 里做，不占主线程**。预生成队列一次只做一个：本次行动要召唤的最先；然后是我方手牌、我方牌库、场上单位；队列空了以后，只要战场在显示，就在空闲时依次生成其余角色。敌方隐藏的手牌和牌库从不读取，它们在被打出时生成，或在空闲时生成。新角色的着色器用 `compileAsync` 并行编译完才出场，所以出场不会卡帧。**打出的牌直接变成角色（用户 2026-09-24 提议）：** 手牌里的牌（`#hand` 变化时）和一次行动将要召唤的牌（`EmberFX.present` 开始时）会提前在 worker 里烘焙（`EmberMiniatures.prewarm`）。卡牌飞到位后，令牌一出现就带上 `.miniature-pending`，原画立刻压暗成底座，从不显示平面卡图；角色随即在令牌上“拼合”出场（落位后约 0.65 秒站定），不再等整段序列结束。没预热到的角色（比如亡语召唤）也是一样：底座先暗，烘焙完就拼合。角色构建失败时去掉底座，退回平面令牌。
- 角色永远不改变规则状态；卡名、费用、攻血仍是 DOM。
- **单位就是角色（用户 2026-09-25）：** 有体素角色的随从上场后不再显示卡牌。令牌（`.miniature-pending` / `.miniature-ready`）的原画、边框、底板、投影和状态框全部隐藏，只保留为点击、拖拽、焦点与无障碍目标；攻击力和生命值变成底座前方的两枚徽章。悬停或长按仍会弹出完整卡牌。
- **站位：** 令牌框就是站位，由布局按行和序号固定摆放，角色站在令牌高度的 60% 处（`FOOT`）。舞台在站位上画一块八边形石质底座（半径 = 0.4 × 令牌宽）：我方铜边、敌方铁边，嘲讽是加宽的白石边；可攻击绿色、可选为目标橙红、冻结冰蓝、圣盾金色，状态都显示为底座外圈的光。战场背景不再为这些单位画卡牌形状的接触阴影。
- **尺寸：** 角色按自己站位处相机的实际像素比例缩放，远排和近排的角色相对令牌一样大。高度不超过令牌顶端以上 0.5 个令牌高（`RISE`），宽度不超过 1.3 个令牌宽，不伸进对面那一行、顶栏或邻位。
- **层级：** 舞台画布在随从层（z 7）之下（z 6），所以徽章、睡眠 Z、伤害数字永远压在角色上面。
- **朝向：** 双方互相面对。我方朝敌方（镜头看到 3/4 背面），敌方朝我方，都略微转向对面一行的中间；攻击时转向目标。
- **不再使用的卡牌形状演出：** 这些单位不放 fx2 的召唤光门（拼合出场代替）、DOM 死亡残影和边缘光线（碎块代替）。
- **英雄的固定座位：** 英雄角色的画布恰好盖在头像窗口（`.portrait-frame`）上，按拱形裁切，角色站在窗口底边、头顶在窗内。它不越出头像框，不挡战场、血量、护甲、武器或英雄技能。
