# 参考页面档案

只读取当前任务相关页面。每页以当前实现摘要为入口，原始素材与完整时间线用链接关联，不复制大图或重复写日志。

下表每份档案都有一节 2026-09-13 / 2026-09-14 的磨砂青岩（slate）记录，写明该页在默认皮肤下的实装、几何与验收；更早的银蓝／胡桃木条目属于历史版本。跨页面的令牌与组件语言见 [SLATE_DESIGN_SYSTEM.md](../SLATE_DESIGN_SYSTEM.md)。

| 页面 | 当前档案 | 用途 |
| --- | --- | --- |
| 酒馆主页 | [home.md](home.md) | 已实装；按钮/图标、收藏层次、导航高光与真实流程 |
| 英雄选择 | [heroes.md](heroes.md) | 当前实装、真实入口与对照证据 |
| 冒险地图 | [map.md](map.md) | 当前实装、真实入口与对照证据 |
| 桌面战斗 | [battle-desktop.md](battle-desktop.md) | 当前实装、真实入口与对照证据 |
| 图鉴与构筑 | [library.md](library.md) | 当前实装、真实入口与对照证据 |
| 契约 | [contracts.md](contracts.md) | 当前实装、真实入口与对照证据 |
| 起手换牌 | [mulligan.md](mulligan.md) | 当前实装、真实入口与对照证据 |
| 遗物奖励 | [rewards.md](rewards.md) | 当前实装、真实入口与对照证据 |
| 胜利结算 | [result.md](result.md) | 当前实装、真实入口与对照证据 |
| 设置 | [settings.md](settings.md) | 当前实装、真实入口与对照证据 |
| 旅人手册 | [guide.md](guide.md) | 当前实装、真实入口与对照证据 |
| 手机竖屏战斗 | [battle-portrait.md](battle-portrait.md) | 当前实装、真实入口与对照证据 |
| 手机横屏战斗 | [battle-landscape.md](battle-landscape.md) | 当前实装、真实入口与对照证据 |
| 二级状态 | [secondary.md](secondary.md) | 当前实装、真实入口与对照证据 |

新页面使用 [TEMPLATE.md](TEMPLATE.md)。页面 ID 用稳定英文短名，图片和实测输出放项目素材/输出目录；档案内链接到具体文件，不以端口 URL 或临时系统路径作为唯一来源。
