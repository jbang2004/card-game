...
## 触摸查看与战斗提示（2026-09-22）

- 英雄预览、起手换牌、发现选项和详情卡支持手指横向抚摸改变卡片角度；列表中的纵向手势仍交给浏览器滚动。抚摸超过 9px 不再触发选牌点击，独立点按保持原操作。
- 图鉴放大卡、神契前景卡和战场长按详情支持直接触摸倾斜，松手 / 取消触摸时归位。神契超过 30px 的横滑仍切换一张，向下超过 90px 仍收回。普通手牌保持横滑和拖放出牌。
- 召唤疲劳使用独立的三个立体 Z，与关键词标记共存；冻结显示雪花，可行动后移除 Z。减少动态设置保留静态标记。
- 放置与瞄准使用带厚侧边、亮色棱面的大箭头；红色分段带状箭身通过空间投影表达俯仰，落点及合法性判断保持原逻辑。
- 专项回归：`tests/e2e/touch-card-feedback.spec.cjs`，浏览器原生触摸事件模拟，不等同于实体手机验收。
