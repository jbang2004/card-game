...
- 卡牌在悬停、焦点和选中状态下都保持 `touch-action: pan-x`；向上拖动和长按扶起仍由出牌控制器处理。
- 点按只选中 / 瞄准，再点击战场空位或合法目标确认；上拖释放可以直接出牌。静止长按可用手牌是扶起拖动，不是打开详情。
- 旋转与滑动不修改对局状态。桌面手牌布局保持不变。
- 回归见 `tests/e2e/mobile-hand-layout.spec.cjs`、`hand-drag.spec.cjs`、`card-reading.spec.cjs`、`ui-viewport-switch.spec.cjs`。
