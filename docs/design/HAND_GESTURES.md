...
- 移动端神契使用英雄区独立 44×44 入口，不与普通手牌同栏；点击仍打开完整诸神卡牌与仪式进度。
- 手牌左右边缘只在对应方向尚有隐藏内容时显示渐隐箭头。提示不拦截触摸，保留原生横滑与卡牌拖拽；滚动事件只更新缓存位置，不读取布局。
- 通知沿用半透明磨砂材质，以较亮、较粗的文字强调消息；桌面状态槽宽 420px。
...
- 全尺寸共用顶栏中央的回合/通知槽：消息出现时回合向上翻出，所有消息及操作结束后向下翻回。短消息优先于操作说明，操作说明优先于回合。竖屏不再另设第二行，声音与设置从菜单进入。减少动态设置下直接切换。
