...
`rules/state.js` 只校验当前 version 2 存档，`platform/storage.js` 独立处理 JSON 与不可用存储。保留：
...
统一入口 `npm run test:release`：构建、Node 规则/素材/架构测试、Playwright。原 Python 内存页面回归已退役。历史文件哈希检查不再阻止合法改卡；旧行为快照已退役。当前规则测试覆盖全部 62 张卡的确定性结算和可恢复状态，具体效果由独立行为测试断言。
...
卡组集合继续使用 `emberfall.deck.v1`，内容仅接受 version 2 命名集合；旧数组不迁移。格式不支持或损坏时停止读取和保存，收藏提供显式“重建测试卡组收藏”。所有牌组遵守当前职业限制。战役存档也只接受 version 2；旧对局失效后重新选择英雄。见 [设计记录](design/CONTENT_DECK_MODEL.md)。
